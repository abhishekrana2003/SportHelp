    ...
    return (    
        <div className="border-[.5px] bg-white border-gray-300 cursor-default flex justify-center items-center text-center h-[50px] w-[150px]">
            {props.type === "time" ? `${props.data < 10 ? `0${props.data}` : props.data }:00 - ${props.data+1 < 10 ? `0${props.data+1}` : props.data+1}:00` :
             props.type === "courtHeader" ? <div className ="h-full w-full flex justify-center items-center font-semibold" >{`Court ${props.data}`}</div> :
             ...
             <div className="h-full w-full">
                ...
                {props.status === "available" ? 
                    <div className="relative flex justify-center items-center h-full w-full">
                       ...
                       {showAddBooking &&  <AddBooking slotTiming={`${props.data < 10 ? `0${props.data}` : props.data }:00 - ${props.data+1 < 10 ? `0${props.data+1}` : props.data+1}:00`} toggleVisible = {handleAdd}/>}
                    </div> :
