...
export default function Calendar({ bookings, courts,selectedDate }){
    return (
        <div className=" h-[60%] max-w-[90%] flex border-[.5px] shadow-lg rounded overflow-scroll">
            ...
            {courts.length!==0?courts.map((court) => (
                <CourtColumn selectedDate = {selectedDate} key={court._id} id = {court._id} courtNumber={court.name} bookings={bookings.filter(b => b.court === court._id)} />
            )):
